    ...
    switch (action.type) {
        ...
        case REMOVE_FROM_CART:
            ...
            if(foundItem.quantity ===1){
                updatedCart = state.cart.filter((cartItem) => cartItem.product._id !== selectedProductId)
            }else{
                updatedCart = state.cart.map((cartItem) => cartItem.product._id=== selectedProductId ? {...cartItem,quantity: cartItem.quantity - 1} : cartItem)
            }
...
export const CartContextProvider = ({children}) => {
    ...
    return <cartContext.Provider value={{addToCart,removeFromCart,cartState, saveTocart}}>{children}</cartContext.Provider>
}